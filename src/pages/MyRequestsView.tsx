...
import { FileText, Clock, CheckCircle2, XCircle, Undo2 } from "lucide-react";
import { StatCards } from "../components/StatCards";
import { RequestList } from "../requests/RequestList";
import { DrillDownModal } from "../requests/DrillDownModal";

// ============ MY REQUESTS ============
export function MyRequestsView({ user, requests, budgets, pos, suppliers, saveSuppliers, poCounter, piCounter, saveRequests, saveBudgets, savePOs, savePOCounter, savePICounter, addNotifications, showToast }) {
  ...
  const returned = myItems.filter(r => r.status === "Returned for Changes");
  const pending = myItems.filter(r => !["Paid", "Rejected", "Cancelled", "Active", "Approved", "Closed", "Returned for Changes"].includes(r.status));
  ...
  return (
    <div>
      <StatCards items={[
        ...
        { label: "Returned", value: returned.length, color: "amber", icon: Undo2, onClick: () => handleCardClick(returned, "Returned for Changes") },
