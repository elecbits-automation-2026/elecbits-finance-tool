...
import { Coins, Target, CheckCircle2, Pencil } from "lucide-react";
import { DEPARTMENTS, RD_MONTHLY_CAP } from "../constants";
import { getRDAllocation, getRDUsageForDeptMonth } from "../lib/finance";
...
export function RDAllocationView({ user, budgets, requests, saveBudgets, addNotifications, showToast }) {
  ...
  async function save() {
    ...
    // Deterministic id => replace any existing allocation for this dept+month.
    await saveBudgets([record, ...budgets.filter(b => b.id !== id)]);
    setSaving(false);
    setForm({ ...form, amount: "" });
