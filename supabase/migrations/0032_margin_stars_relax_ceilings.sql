-- 0032_margin_stars_relax_ceilings.sql
-- Margin Stars: the 20% margin becomes a graded star cost, not a hard wall.
--   * budgets_guard: a Client EXTENSION may now commit up to 100% of the client
--     order value (was 80%). The FIRST Client budget still can't cross 80% on its
--     own; dipping past 80% happens via an extension (justification + star cost are
--     enforced client-side). Hard stop at 100%.
--   * po_caps_guard: PO 20%-margin ceiling relaxed from 80% to a hard stop at 100%
--     of COV (the project-budget cap remains the real spend limit).
-- Depends on project_client_order_value() from 0031 (already live). Full function
-- bodies (CREATE OR REPLACE requires the whole function).

-- budgets_guard (Client extension cap 80% -> 100%) -------------------------
...
-- po_caps_guard (PO margin ceiling 80% -> hard stop at 100%) ---------------
