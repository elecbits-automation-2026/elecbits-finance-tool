-- 0031_pi_driven_client_order_value.sql
-- A Client project's order value is now DERIVED from its APPROVED PIs (live), not
-- typed on the budget. This replaces the trusted-submitted clientOrderValue in the
-- server guards with a computed value:
--   * project_client_order_value(project_id) — Σ approved/closed PICreate amounts.
--   * budgets_guard: a Client budget requires >=1 approved PI and is capped at 80%
--     of that live value (extensions too; legacy stored value as fallback).
--   * po_caps_guard: the 20% margin ceiling uses the live value (legacy fallback).
-- Full function bodies (Postgres CREATE OR REPLACE requires the whole function).
-- Mirrors src/lib/finance.ts getProjectClientOrderValue + the client forms.
...
-- budgets_guard (Client + Extension ceilings now from approved PIs) ---------
  ...
  if tg_op = 'INSERT' then
    ...
    if new.type = 'Extension' then
      ...
      if parent->>'projectType' = 'Client' then
        ...
        if (parent->>'amountINR')::numeric + ext_total + amt > cov * 0.80 then
...
-- po_caps_guard (20% margin ceiling now from approved PIs) ------------------
  ...
  if coalesce(d->>'isProject','') = 'true' then
    ...
    if coalesce(b->>'projectType','') = 'Client' and cov > 0 then
      ...
      if total_after > ceiling + tol then
        raise exception 'po: breaches 20%% margin — max PO commitments ₹% (80%% of client order value ₹% from approved PIs)', ceiling, cov;
      end if;
    end if;
    return new;
  else
