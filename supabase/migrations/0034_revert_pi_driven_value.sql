-- 0034_revert_pi_driven_value.sql
-- Reverts the Start Project doc chain (0033). Projects are driven by PIs again:
--   * project_client_order_value(project_id) → back to sum of approved/closed PIs.
--   * budgets_guard → StartProject INSERT branch removed (unknown types rejected).
-- Restores the state after 0031/0032. po_caps_guard is unchanged (still calls
-- project_client_order_value). Full function bodies.

-- client-order value back to approved PIs ----------------------------------
...
-- budgets_guard without the StartProject branch ---------------------------
