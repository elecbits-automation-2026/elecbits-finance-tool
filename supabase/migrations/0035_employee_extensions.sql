-- 0035_employee_extensions.sql
-- Allow employees (not just department heads) to raise budget EXTENSIONS. Monthly
-- budgets stay heads-only. The parent-project checks (must be an active project in
-- the raiser's own department) still restrict which project can be extended, so
-- opening the role gate is safe. Full budgets_guard body (CREATE OR REPLACE).
