-- 0014_onetime_budget.sql
-- Adds a "One-Time Budget" variant of a Project budget (projectType = 'OneTime').
-- It is a one-off approved spend: no client order value and no 80% cap, but the
-- same department/role raise gates, the same unique-projectId guard, and the same
-- approval workflow (Dept Head -> Finance Head -> VP/CEO by amount) as other
-- project budgets. The client auto-generates a unique projectId for it so it stays
-- spendable via the project payment flow.
--
-- Only change vs the prior budgets_guard() is the new `elsif projectType = 'OneTime'`
-- branch inside the INSERT/Project section; the whole function is re-created here
-- because Postgres has no way to patch a single statement inside a function body.
